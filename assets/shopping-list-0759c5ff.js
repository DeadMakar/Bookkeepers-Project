import{K as p,N as H}from"./modal-local-storage-9e1ba06f.js";const r={ShopUlEl:document.querySelector(".shopping-list-container"),ShopStubEl:document.querySelector(".shopping-list-container"),shopRemoveBtnEl:document.querySelector(".shopping-delete-btn")},m="/Bookkeepers-Project/assets/oooops-mob-1x-b0b9c805.png",a="/Bookkeepers-Project/assets/oooops-mob-2x-5c7a8c33.png",g="/Bookkeepers-Project/assets/oooops-tab-1x-aa5b2255.png",c="/Bookkeepers-Project/assets/oooops-tab-2x-6e7faaa3.png",i="data:image/png;base64,",n="data:image/png;base64,",o="data:image/png;base64,",s="data:image/png;base64,",l="/Bookkeepers-Project/assets/symbol-defs-c8f30ebc.svg#icon-delete";function x(A){const t=A.map(h=>`<li class="shopping-item">
      <div class="shopping-list-item">
        <div class="shopping-book-overlay">
          <img
            class="shopping-book-img"
            src="${h.book_image}"
            alt="${h.title}"
            width="100"
            height="142"
          />

      <p class="shopping-book-overlay-text">quick view</p>
    </div>

    <div class="shopping-item-container">
      <h3 class="shopping-book-title">${h.title}</h3>

      <p class="shopping-book-category-text">${h.list_name}</p>

      <p class="shopping-book-description">${h.description}</p>

      <p class="shopping-book-author">${h.author}</p>

      <div class="shopping-link-wrap">
        <a
          href="${h.buy_links[0].url}"
          target="_blank"
          rel="noopener noreferrer"
          class="shopping-link"
        >
          <picture>
            <source
              srcset="
                ${i},
                ${n}
              "
              media="(min-width: 1440px)"
            />
            <source
              srcset="
                ${i},
                ${n}
              "
              media="(min-width: 768px)"
            />
            <source
              srcset="
                ${i},
                ${n}
              "
              media="(max-width: 768px)"
            />
            <img
              class="shopping-img-amazon"
              src="${i}"
              alt="amazon"
              width="32"
              height="11"
            />
          </picture>
        </a>
        <a
          href="${h.buy_links[1].url}"
          target="_blank"
          rel="noopener noreferrer"
          class="shopping-link"
        >
          <picture>
            <source
              srcset="
              ${o}, 
              ${s}"
              media="(min-width: 1440px)"
            />
            <source
              srcset="
              ${o}, 
              ${s}"
              media="(min-width: 768px)"
            />
            <source
              srcset="
              ${o}, 
              ${s}"
              media="(max-width: 768px)"
            />
            <img
              class="shopping-img-book"
              src="${o}"
              alt="book"
              width="11"
              height="11"
            />
          </picture>
        </a>
      </div>
    </div>
  </div>
  <button class="shopping-delete-btn" type="button" data-card-id ="${h._id}">
    <svg class="shopping-delete-btn-icon" width="16" height="16">
      <use class="shopping-icon" href="${l}" ></use>
    </svg>
  </button>
</li>
`).join("");r.ShopUlEl.insertAdjacentHTML("beforeend",t)}function d(){const A=`<li class="shopping-error-wrap">
      <p class="shopping-error-text">
        This page is empty, add some books and proceed to order.
      </p>
      
        <picture>
          <source
            srcset="
              ${g},
              ${c}
            "
            media="(min-width: 1440px)"
          />
          <source
            srcset="
              ${g},
              ${c}
            "
            media="(min-width: 768px)"
          />
          <source
            srcset="
              ${m},
              ${a}
            "
            media="(max-width: 768px)"
          />
          <img
        class="shopping-error-img"
        src="${m}"
        alt="books"
        width="265"
        height="198"
      />
        </picture>
    </li>`;r.ShopStubEl.innerHTML=A}let e=JSON.parse(localStorage.getItem(p))||[];r.ShopUlEl.addEventListener("click",w);Q();function Q(){try{if(e.length===0){d(e);return}x(e)}catch(A){console.error("Error:",A),H.Notify.failure("Sorry, there are no images matching your search query. Please try again.")}}function w(A){if(A.target.className!="shopping-delete-btn")return;let t=e.findIndex(h=>h===A);e.splice(t,1),localStorage.setItem(p,JSON.stringify(e)),window.location.href=""}
