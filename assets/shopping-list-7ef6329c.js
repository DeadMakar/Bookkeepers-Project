import{K as H,N as l}from"./modal-local-storage-7cb1ebbf.js";const t={ShopUlEl:document.querySelector(".shopping-list-container"),ShopStubEl:document.querySelector(".shopping-list-container"),shopRemoveBtnEl:document.querySelector(".shopping-delete-btn")},m="/Bookkeepers-Project/assets/oooops-mob-1x-b0b9c805.png",x="/Bookkeepers-Project/assets/oooops-mob-2x-5c7a8c33.png",g="/Bookkeepers-Project/assets/oooops-tab-1x-aa5b2255.png",c="/Bookkeepers-Project/assets/oooops-tab-2x-6e7faaa3.png",i="data:image/png;base64,",s="data:image/png;base64,",o="data:image/png;base64,",r="data:image/png;base64,",d="/Bookkeepers-Project/assets/symbol-defs-c8f30ebc.svg#icon-delete";function Q(h){const n=h.map(A=>`<li class="shopping-item">
      <div class="shopping-list-item">
        
          <img
            class="shopping-book-img"
            src="${A.book_image}"
            alt="${A.title}"
            width="100"
            height="142"
          />
          

    <div class="shopping-item-container">
      <h3 class="shopping-book-title">${A.title}</h3>

      <p class="shopping-book-category-text">${A.list_name}</p>

      <p class="shopping-book-description">${A.description}</p>

      <p class="shopping-book-author">${A.author}</p>

      <div class="shopping-link-wrap">
        <a
          href="${A.buy_links[0].url}"
          target="_blank"
          rel="noopener noreferrer"
          class="shopping-link"
        >
          <picture class="picture-amazon">
            <source
              srcset="
                ${i},
                ${s}
              "
              media="(min-width: 1440px)"
            />
            <source
              srcset="
                ${i},
                ${s}
              "
              media="(min-width: 768px)"
            />
            <source
              srcset="
                ${i},
                ${s}
              "
              media="(max-width: 768px)"
            />
            <img
              class="shopping-img-amazon"
              src="${i}"
              alt="amazon"
              width="32"
              height="11"
            />
          </picture>
        </a>
        <a
          href="${A.buy_links[1].url}"
          target="_blank"
          rel="noopener noreferrer"
          class="shopping-link"
        >
          <picture class="picture-book">
            <source
              srcset="
              ${o}, 
              ${r}"
              media="(min-width: 1440px)"
            />
            <source
              srcset="
              ${o}, 
              ${r}"
              media="(min-width: 768px)"
            />
            <source
              srcset="
              ${o}, 
              ${r}"
              media="(max-width: 768px)"
            />
            <img
              class="shopping-img-book"
              src="${o}"
              alt="book"
              width="11"
              height="11"
            />
          </picture>
        </a>
      </div>
    </div>
  </div>
  <button class="shopping-delete-btn" type="button" data-id="${A._id}">
    <svg class="shopping-delete-btn-icon" width="16" height="16">
      <use href="${d}" ></use>
    </svg>
  </button>
</li>
`).join("");t.ShopUlEl.insertAdjacentHTML("beforeend",n)}function f(){const h=`<li class="shopping-error-wrap">
      <p class="shopping-error-text">
        This page is empty, add some books and proceed to order.
      </p>
      
        <picture>
          <source
            srcset="
              ${g},
              ${c}
            "
            media="(min-width: 1440px)"
          />
          <source
            srcset="
              ${g},
              ${c}
            "
            media="(min-width: 768px)"
          />
          <source
            srcset="
              ${m},
              ${x}
            "
            media="(max-width: 768px)"
          />
          <img
        class="shopping-error-img"
        src="${m}"
        alt="books"
        width="265"
        height="198"
      />
        </picture>
    </li>`;t.ShopStubEl.innerHTML=h}let e=JSON.parse(localStorage.getItem(H))||[];t.ShopUlEl.addEventListener("click",w);a();function a(){try{if(e.length===0){f(e);return}Q(e)}catch(h){console.error("Error:",h),l.Notify.failure("Sorry, there are no images matching your search query. Please try again.")}}function w(h){if(!h.target.classList.contains("shopping-delete-btn"))return;h.target.closest("li.shopping-item");const n=h.target.dataset.id,A=e.findIndex(p=>p._id===n);A!==-1&&(e.splice(A,1),localStorage.setItem(H,JSON.stringify(e)),t.ShopStubEl.innerHTML="",a())}
