...
`).join("");t.ShopUlEl.insertAdjacentHTML("beforeend",s)}function f(){const A=`<li class="shopping-error-wrap">
    ...
    </li>`;t.ShopStubEl.innerHTML=A}let i=JSON.parse(localStorage.getItem(a))||[];t.ShopUlEl.addEventListener("click",w);c();function c(){try{if(i.length===0){f(i);return}Q(i)}catch(A){console.error("Error:",A),l.Notify.failure("Sorry, there are no images matching your search query. Please try again.")}}function w(A){if(!A.target.classList.contains("shopping-delete-btn"))return;A.target.closest("li.shopping-item");const s=A.target.dataset.id;console.log(A.target.dataset.id);const h=i.findIndex(H=>H._id===s);h!==-1&&(i.splice(h,1),localStorage.setItem(a,JSON.stringify(i)),t.ShopStubEl.innerHTML="",c())}
